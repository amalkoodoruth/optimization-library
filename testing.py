...
class TestProblemB(unittest.TestCase):
    ...
    def test_sd(self):
        x = np.array([[10], [10]])
        start = time.time()
        x_values = optimization.steepest_descent(self.p, x, tol=1e-4, hist=True)
        end = time.time()
        g = np.array([np.linalg.norm(self.p.get_grad(x_values[i]))
            for i in range(len(x_values))])
        fig = plt.figure()
...
class TestProblemE(unittest.TestCase):
    ...
    #     x0 = np.array([[1], [1]])
    #     start = time.time()
    #     x_opt = optimization.augmented_lagrange(self.p, x0, tol=1e-4, tol_cost=1e-4, hist=True)
    #     end = time.time()
    #     g = np.array([np.linalg.norm(self.p.get_grad(x_opt[i]))
    #                   for i in range(len(x_opt))])
    #     c_i = np.array([np.linalg.norm(np.minimum(self.p.get_ineq_const(x_opt[i]), 0))
    #                     for i in range(len(x_opt))])
    #     fig = plt.figure()
...
class TestProblemEEq(unittest.TestCase):
    ...
    def setUp(self):
        v = lambda x: -x[0, 0] * x[1, 0]
        h1 = lambda x: -x[0, 0] - x[1, 0]**2 + 1
        self.p = optimization.Problem(v, eq_const=[h1])
...
class TestProblemF(unittest.TestCase):
    ...
    def test_penalty_function(self, tol=1e-4, tol_const=1e-4):
        x0 = np.array([[1], [1]])
        x = optimization.penalty_function(self.p, x0)

    def test_inv_barrier_function(self, tol=1e-4, tol_const=1e-4):
        x0 = np.array([[1], [1]])
        x = optimization.barrier_function(self.p, x0, mode='inv')

    def test_log_barrier_function(self, tol=1e-4, tol_const=1e-4):
        x0 = np.array([[1], [1]])
        x = optimization.barrier_function(self.p, x0, mode='log')

    def test_aug_lag(self):
        x0 = np.array([[2.1], [0.1]])
        x = optimization.augmented_lagrange(self.p, x0, tol=1e-4, tol_cost=1e-4)

    def test_lag_new(self):
        x0 = np.array([[0.7], [0.7]])
        x_values = optimization.lagrange_newton(self.p, x0, tol=1e-4)
        print(x_values)
